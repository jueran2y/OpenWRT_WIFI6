...
function add_vif_into_lan(vif)
    ...
    if not string.match(brvifs, esc(vif)) then
        ...
        if vif == "apcli0" or vif == "apclix0" then
            local res = mtkwifi.read_pipe("uci show dhcp.lan.ignore 2>&1")
            if string.match(res, "Entry not found") ~= nil then
                nixio.syslog("debug", "DHCP LAN IS Start And Do not Add Apcli to Br-lan")
            elseif string.match(res, "dhcp.lan.ignore=\'1\'") then
                nixio.syslog("debug", "DHCP LAN is Stop And Add Apcli Into Br-lan")
                brvifs = brvifs.." "..vif
            ...
            nixio.syslog("debug", vif.." start up and add in br-lan cmd:".."brctl addif br-lan "..vif)
            brvifs = brvifs.." "..vif
            os.execute("uci set network.lan.ifname=\""..brvifs.."\"")
            os.execute("uci commit")
            --os.execute("brctl addif br-lan "..vif)
...
function mt7615e_up(devname)
    ...
    if l1parser and devs then
        ...
        nixio.syslog("debug", "mt7615e_up1 dev name: "..dev.main_ifname)
        if mtkwifi.exists("/sys/class/net/"..dev.main_ifname) then
            nixio.syslog("debug", "mt7615e_up1: ifconfig "..dev.main_ifname.." up")
        ...
        do
            ...
            then
                nixio.syslog("debug", "mt7615e_up2: ifconfig "..vif.." up")
    ...
    elseif mtkwifi.exists("/etc/wireless/mt7615e/"..devname..".dat") then
        for _, pre in ipairs(vif_prefix) do
            ...
            if mtkwifi.exists("/sys/class/net/"..pre.."0") then
                nixio.syslog("debug", "mt7615e_up3: ifconfig "..pre.."0 up")
            ...
            do
                ...
                if string.match(vif, pre.."[1-9]+") then
                    nixio.syslog("debug", "mt7615e_up4: ifconfig "..vif.." up")
                    os.execute("ifconfig "..vif.." up")
                    add_vif_into_lan(vif)
                    if wifi_services_exist then
                        miniupnpd_chk(devname, vif, true)
    ...
    --[[
    if dev.main_ifname == "ra0" then
        os.execute("iwpriv rax0 set led_setting=0-0-0-0-0-0-0-5 ")
    end

    if dev.main_ifname == "ra0" then
        os.execute("iwpriv rax0 set led_setting=1-0-0-0-0-0-0-5 ")
    end
    ]]--
...
function mt7615e_down(devname)
    ...
    --[[
    if devname == "ra0" then
        os.execute("iwpriv rax0 set led_setting=0-0-0-0-0-0-0-0 ")
    end

    if devname == "ra0" then
        os.execute("iwpriv rax0 set led_setting=1-0-0-0-0-0-0-0 ")
    end
    ]]--
