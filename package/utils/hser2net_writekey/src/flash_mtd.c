...
int flash_write_key(int offset, key_b *p, int len)
{
	int i, o, fd, off, addr, sz;
	unsigned char *buf;
	struct erase_info_user ei;
	...
	for (i = 0, addr = 0; i < NUM_INFO; i++) {
		//printf("off :%x addr + info[i].size: %x name:%s\n",offset, addr + info[i].size, info[i].name);
		if (addr <= off && off < addr + info[i].size) {
			sz = info[i].erasesize;
			buf = (unsigned char *)malloc(sz);
			if (buf == NULL) {
			...
			lseek(fd, o, SEEK_SET);
		{
			int i;
			for(i = 0; i < len; i++) {
				*(buf + (off - o)+p[i].addr) = (unsigned char)p[i].value;
				printf("Write %0X to %0X\n", (int)p[i].value, (p[i].addr+off));
			}
		}
			if (write(fd, buf, sz) == -1) {
				fprintf(stderr, "failed to write mtd%d\n", i);
